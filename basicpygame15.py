#ทดสอบการชนของวัตถุในเกม ************
#รับค่าจากkeyboard*****

#****เราอยากทำอะไร = ทดสอบการชนของวัตถุว่าแบบ paddle ซ้อนทับ ball หรือยัง  **********
#if paddle_rect.colliderect(ball_rect):
    #print("hit") หรือเปลี่ยนตำแหน่งของ ballใหม่ โดยการสุ่ม ball_rect ใหม่ ใช้ random ต้อง import random 
    #ball_rect.left = random.randint(0,SCREEN_W - 32) ใช้คําสั่งนี้


#ต่อยอดมาจาก basicpygame13.py
...
#ใช้แบบนี้ เพราะ อีกแบบนี้ไม่สามารถทํางานได้
paddle = pygame.image.load("image/paddle.png")
ball = pygame.image.load("image/ball.png")
...
while running:
    ...
    if paddle_rect.colliderect(ball_rect):
            # print("hit")
        ball_rect.left = random.randint(0,SCREEN_W - 32) #การสุ่ม ball_rect ใหม่ 
            ##,randint คือการสุ่มตัวเลข ,=32 คือให้ห่างจากขอบหน้าจอเกม 32
    keys = pygame.key.get_pressed()
    if keys[pygame.K_UP] and paddle_rect.top > 0: #คำสั่งเปลี่ยนตำแหน่ง โดยใช้คีย์บอร์ด กดปุ่มขึ้น
    ...
    # #แสดงขอบเขตวัตถุ 
    # pygame.draw.rect(screen,GREEN,paddle_rect,2) #2 คือ ขนาดขอบเขตวัตถุ ที่เราวาด หรือกรอบวัตถุ
